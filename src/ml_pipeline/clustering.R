...
# cluster.fix_feature_importance <- function(model_feature_importance,
#                                            cluster_objs) {
#   if (is.null(cluster_objs)) {  # no clustering, just create the desired dataframe
#     model_feature_importance[, 'representative'] <- NA
#     return(model_feature_importance)
#   }
#   
#   # The output of this loop is a list with an item for each cluster
#   # Each list item contains a named vector of the members of the cluster and their importance
#   clusters_importances <- lapply(cluster_objs, function(cluster) {
#     # get the entry of the representative in the rf model, and check that it exists
#     rep_entry <- filter(model_feature_importance, feature == cluster$representative)
#     if (nrow(rep_entry) > 0) {
#       cluster_features_importance <- lapply(cluster$features, function(feature_name) {
#         return(list(feature = feature_name,
#                     importance = rep_entry$importance,
#                     pvalue = rep_entry$pvalue,
#                     representative = rep_entry$feature))
#       })
#       return(cluster_features_importance)
#     }
#   })
# 
#   # And now let's flat these nested lists into one list and create a dataframe
#   clusters_importances <- Reduce(c, clusters_importances)
#   feature_importance_df <- bind_rows(clusters_importances)
